// Draw routines for terrain-hazard patches: Tide Pool water, Rocky Shore stone, Neon Kelp
// weed beds, and the King Crab boss's enrage-phase floor fissures. Split out of graphics.rs
// (which was in "crisis" territory per AGENTS.md) as a self-contained cluster: these four
// functions share the same pool/fissure input shapes and batching techniques and call into
// each other (draw_tide_pools dispatches to draw_rock_patches/draw_kelp_patches by TerrainKind).

use crate::graphics::{cached_stroke_circle, stroke_circle_key, stroke_circle_mesh_for_key, unit_circle, unit_line};
use ggez::Context;
use ggez::glam::Vec2;
use ggez::graphics::{BlendMode, Canvas, Color, DrawParam, InstanceArray, Mesh};
        ...
        POOL_RIPPLE_GROUPS.with(|ripple_groups_cell| -> ggez::GameResult {
            ...
            POOL_RIM_INSTANCES.with(|inst_cell| -> ggez::GameResult {
                ...
                for (key, params) in rim_groups.iter() {
                    ...
                    let mesh = stroke_circle_mesh_for_key(*key);
