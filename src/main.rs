...
const UPGRADE_POOL: [UpgradeId; 9] = [
    UpgradeId::BeamFocus,
...
impl UpgradeId {
    ...
    fn card(self) -> (&'static str, &'static str, &'static str, u8, u8, u8) {
        match self {
...
thread_local! {
    ...
    static UPGRADE_SCREEN_CACHE: RefCell<Option<(
        ([usize; 3], u32, u32, u32, u32), // which three options are offered + the four tool ranks
        Text, f32,         // title text + width
        Text, f32,         // hint text + width
        [(Text, f32, Text, f32, Text, f32, Text, f32, Text, f32); 3], // per card: icon, name, rank, desc, key (each + width)
    )>> = RefCell::new(None);
...
impl MainState {
    ...
    fn roll_upgrade_offer(&mut self) {
    ...
    /// choice is the 1-based card slot the player picked (1..=3); it indexes into
    /// self.offered_upgrades (the three options rolled at queue time). Each option applies its
    /// boon AND, for the tradeoff cards, its cost — so a pick specializes the run rather than
    /// piling on pure power. speed_mult and catch_radius_upgrade are floored so repeated tradeoffs
    /// can't drive the player to a standstill or a zero-reach net.
