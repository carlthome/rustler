...
// Upgrade cadence constants (UPGRADE_FIRST_AT, UPGRADE_STEP) and the whole upgrade model now live
// in src/upgrade.rs; UPGRADE_FIRST_AT is imported below for the score-threshold reset sites.
...
// UpgradeId, its card() metadata, and UPGRADE_POOL now live in src/upgrade.rs (imported above).
