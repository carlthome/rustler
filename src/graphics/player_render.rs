...
fn build_cosmetics_meshes(
    ...
    let rect_mesh =
        |ctx: &mut Context, rect: Rect, color: Color| -> ggez::GameResult<(Mesh, DrawParam)> {
    ...
    match skin.hat {
        ...
        Hat::Cowboy => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.32, ht.y + h * 0.04, w * 0.64, h * 0.07),
                brim,
            )?);
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.15, ht.y - h * 0.10, w * 0.30, h * 0.15),
                brim,
            )?);
        }
        Hat::TopHat => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.28, ht.y + h * 0.06, w * 0.56, h * 0.06),
                black,
            )?);
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.14, ht.y - h * 0.20, w * 0.28, h * 0.28),
                black,
            )?);
        }
        Hat::Sombrero => {
        ...
        Hat::Bucket => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.24, ht.y + h * 0.08, w * 0.48, h * 0.05),
                olive,
            )?);
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.18, ht.y - h * 0.02, w * 0.36, h * 0.11),
                olive,
            )?);
        }
        Hat::Bandana => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.26, ht.y + h * 0.06, w * 0.52, h * 0.08),
                red,
            )?);
        ...
        Hat::HardHat => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(ht.x - w * 0.22, ht.y + h * 0.10, w * 0.44, h * 0.04),
                yellow,
            )?);
        }
    }
    ...
    match skin.facial_hair {
        ...
        FacialHair::Beard => {
            out.push(rect_mesh(
                ctx,
                Rect::new(mo.x - w * 0.18, mo.y, w * 0.36, h * 0.22),
                brown,
            )?);
            let uc = unit_circle(ctx)?.clone();
            out.push((
    ...
    match skin.accessory {
        ...
        Accessory::StarBadge => {
            ...
            out.push((
                star,
                DrawParam::default().dest(Vec2::new(sh.x - w * 0.14, sh.y)),
            ));
        }
        Accessory::Monocle => {
            ...
            out.push((
                ring,
                DrawParam::default().dest(Vec2::new(fa.x + w * 0.13, fa.y - h * 0.02)),
            ));
        }
        Accessory::BowTie => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(nx - w * 0.02, ny - h * 0.03, w * 0.04, h * 0.06),
                col(0x22, 0x22, 0x22),
            )?);
        }
        Accessory::NeonChain => {
        ...
        Accessory::Shades => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(fa.x - w * 0.05, fa.y - h * 0.02, w * 0.10, h * 0.02),
                dark,
            )?);
        }
        Accessory::LassoLoop => {
        ...
        Accessory::GoldTooth => {
            ...
            out.push(rect_mesh(
                ctx,
                Rect::new(mo.x - w * 0.02, mo.y - h * 0.01, w * 0.04, h * 0.05),
                col(0xFF, 0xD7, 0x00),
            )?);
        }
    }
