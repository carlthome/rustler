//! Per-crab archetype aura and cleave-effect draws — the persistent per-frame visual
//! identity rings/glows layered under or around each special crab so the player reads its
//! type at a glance: the Armored crab's cold steely shell arc, the Hermit's coppery coil,
//! the attracted-crab and Magnet pull glows, the Thief prowl aura, the Golden sparkle, the
//! Splitter aura, plus the Cleave tool's stakes/slash and the tail-run badge. Extracted
//! from `graphics/mod.rs` to keep that file navigable; these lean on the shared cached
//! meshes and the archetype-ring deferred-draw batching (`defer_archetype_ring` /
//! `flush_archetype_rings`) defined in the parent module (reached here via `use super::*`).
