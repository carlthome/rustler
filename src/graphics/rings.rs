//! Beat-feedback ring, pulse, shockwave and trail effects — the expanding ghost
//! rings, catch shockwaves, catch/call streak trails, fear/tide pulses, and the
//! various on-beat ability rings (whistle, stomp, slam, call, groove-call,
//! downbeat, cycle-preview, centerpiece, catch-next hint, catch-bloom). Extracted
//! from `graphics/mod.rs` to keep that file navigable — these are the transient
//! juice layer drawn over the crabs each frame, all leaning on the shared cached
//! meshes, instance buffers and deferred draw-param thread-locals defined in the
//! parent module (reached here via `use super::*`).
