    ...
    pub(crate) fn update_crabs(&mut self, dt: f32, area: (f32, f32)) {
        ...
        for crab in &mut self.crabs {
            ...
            if crab.is_boss() && !crab.caught {
                let mut ctx = crab_boss_update::BossUpdateCtx {
                    ...
                    armored_positions: &armored_positions,
                    reef_on_field: &mut reef_on_field,
                    reef_boss_pos: &mut reef_boss_pos,
                    reef_hit_landed: &mut reef_hit_landed,
                    boss_broke: &mut boss_broke,
                    boss_enrages: &mut boss_enrages,
                    tide_fires: &mut tide_fires,
                    tide_swells: &mut tide_swells,
                    boss_windups: &mut boss_windups,
                    boss_launches: &mut boss_launches,
                    boss_charge_dust: &mut boss_charge_dust,
                    boss_blocks: &mut boss_blocks,
                    boss_stuns: &mut boss_stuns,
                    hermit_king_reshells: &mut hermit_king_reshells,
                };
                crab_boss_update::update_boss_crab(crab, dt, area, &mut rng, &mut ctx);
                continue;
            }
