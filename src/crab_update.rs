//! Per-frame crab simulation, split out of `main.rs`.
//!
//! `MainState::update_crabs` is the large per-crab update pass run every frame
//! from the main tick. It lives in its own module to keep `main.rs` navigable;
//! it is otherwise unchanged and still operates on `MainState`'s fields, so it
//! stays a `MainState` method via an `impl` block here.
...
use crate::spawnings::spawn_hype_dancer;
