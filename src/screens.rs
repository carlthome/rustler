//! Full-screen and banner overlay drawing — the level title card, the FRENZY/stage-shout
//! banners, the tutorial instruction card, the crab train render pass (with catch-shake), the
//! game-over screen, and the upgrade-choice screen. Extracted from main.rs (part of the main.rs
//! module split); these are the "overlay on top of the world" draws, as opposed to `draw_game`'s
//! world rendering, which stays in main.rs.
//!
//! Because `screens` is a child module of the crate root (`mod screens;` in main.rs), the
//! `impl MainState` block below can freely read the struct's private fields and call its private
//! methods — Rust privacy is module-scoped and a child sees its ancestor's privates.
...
use crate::MainState;
use crate::constants::{BEAT_WINDOW, BOSS_MAX_HEALTH, CRAB_SIZE};
use crate::graphics::{
...
// Vec<usize> allocation that was fired inside draw_crabs_with_shake is eliminated. Grown-but-
// not-shrunk pattern: reaches steady state at max train length and stays there.
thread_local! {
