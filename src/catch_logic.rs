//! The catch/deliver core of the in-run loop, extracted from `main.rs` to keep that
//! file navigable. Three `MainState` methods live here:
//!   * `handle_crab_catching` — the player-body catch: on-beat scoring, streaks, the
//!     Groove Gamble, match-runs, and all the deferred archetype payouts.
//!   * `catch_by_chain` — the conga-line catch: every train link scoops nearby crabs
//!     via a spatial grid.
//!   * `try_deliver_train` — banking the train at the pen: the super-linear payout,
//!     delivery streak, PERFECT-on-beat bonus, and the celebratory feedback.
//!
//! Pure structural move — no behaviour change. These stay `impl MainState` so they
//! keep direct access to the same fields they always used.
...
impl MainState {
    ...
    pub(crate) fn catch_by_chain(&mut self, ctx: &mut Context) {
