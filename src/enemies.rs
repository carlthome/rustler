...
pub enum CrabType {
    ...
    Splitter, // "Splitter Crab" — catching it cleaves your conga train at the midpoint. It's a real arrangement *bet*, gated on timing: catch it ON the beat and the back half BANKS for points (a partial cash-out, with a Jackpot on Goldens/Magnets/a cashed match-run in the slice); WHIFF the beat and the cut botches — the back half scatters free and pays nothing, and any tail match-run is lost. So snag it on the beat to cash a slice at speed, or dodge it to protect a hot run/shape. Reuses the delivery payout + startle-scatter verbs.
