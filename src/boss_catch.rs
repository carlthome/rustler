//! Boss caught-payoff and arena set-pieces: the King Crab / Tide Boss "caught" celebration
//! (score lump-sum, fireworks, camera punch, and the arena healing back to normal), the
//! captured-King live power-up, and the boss-fight arena hazards themselves — cracked-floor
//! fissures, arena flooding, the tail-swallowing fissure bite, and the Tide Boss pulse
//! shockwave with its Magnet-anchor / Golden-slingshot crossovers. Split out of
//! `catch_effects.rs`'s impl MainState — same methods, same behaviour, just grouped by the
//! boss subsystem so the ordinary catch-reward effects stay a separate concern.
