//! The catch-and-bank core loop, lifted out of `main.rs` to keep that file navigable:
//!
//! * `handle_crab_catching` — the per-frame scan that snaps wild crabs onto the tail of the
//!   conga train (match-run/bond scoring, boss/golden/dancer/magnet-shine special cases).
//! * `try_deliver_train` — banking the train at the delivery pen: the super-linear payout,
//!   arrangement bonuses, streak/perfect/gamble multipliers, and the delivery parade.
//!
//! Pure structural move — the two methods are extracted verbatim as a single `impl MainState`
