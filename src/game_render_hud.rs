//! Screen-space HUD rendering for `MainState`.
//!
//! Extracted from `game_render.rs`: the screen-space pass of `draw_game`. After the
//! world-space scene is drawn, `draw_game` switches the canvas to a fixed viewport rect
//! and hands off to `draw_hud`, which paints everything pinned to the screen — weather
...
impl MainState {
    /// Screen-space HUD pass. Called by `draw_game` after it switches the canvas out of
    /// world space; every draw here lands in the fixed viewport (offset only by screen shake).
