...
use crate::controls;
use crate::{
    BEAT_WINDOW, BOSS_DRAIN_RATE, DRUM_ROLL_MAX, PLAYER_SIZE, SLAM_RADIUS, SLOWMO_DURATION,
    STOMP_COOLDOWN, STOMP_MAX_RADIUS, WHISTLE_COOLDOWN, WHISTLE_MAX_RADIUS, WHISTLE_PULL_SPEED,
};

/// The player's beat-synced tool kit: Drum Roll, Call, Cycle/Bubble, Groove Call, Downbeat Slam,
/// whistle/stomp/beam tuning by upgrade rank, and the bot-test helpers that stage/drive these same
/// tools headlessly (force_npc_cross/force_player_cross, bot_fire_events, bot_check_done, and the
/// seek-catch targeting used by the autopilot). Split out of main.rs to keep that file navigable.
impl MainState {
    ...
    pub(crate) fn force_npc_cross(&mut self) {
    ...
    /// rival) is shortcut.
    pub(crate) fn force_player_cross(&mut self) {
