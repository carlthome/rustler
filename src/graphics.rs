...
pub use crate::terrain_fx::{draw_boss_fissures, draw_tide_pools};
...
/// Look up an already-cached stroke-circle mesh by its `stroke_circle_key`, without building one
/// if it's missing. Lets callers outside this module (e.g. terrain_fx's pool rim/ripple flush,
/// which defers the actual mesh fetch to a later grouped pass after `cached_stroke_circle` has
/// already ensured the entry exists) reuse `STROKE_CIRCLE_CACHE` without that static needing to
/// be `pub(crate)` itself.
pub(crate) fn stroke_circle_mesh_for_key(key: (i32, i32)) -> Option<Mesh> {
    STROKE_CIRCLE_CACHE.with(|c| c.borrow().get(&key).cloned())
}
...
/// Draw the three vertical world zones — grass (left), beach (middle), water (right) — with
/// procedural texture and character: varied greens + mowing stripes + tufts + flowers on grass;
/// warm sand + pebbles + shells + ripples on the beach; deep water with time-animated ripple
/// lines, a bright surface strip and foam dots. Zone borders are feathered with blended
/// transition strips rather than hard vertical seams. `time` drives only the water ripple phase.
///
